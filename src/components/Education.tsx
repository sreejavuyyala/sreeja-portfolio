    ...
    <section id="education" className="scroll-mt-20 bg-bg py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2">
          {education.map((entry) => (
            <div key={entry.school} className="rounded-xl border border-slate-light/60 bg-surface p-6">
              <p className="font-mono text-xs uppercase tracking-[0.12em] text-primary">
                {entry.start} — {entry.end}
              </p>
              <h3 className="mt-2 font-display text-lg font-semibold text-ink">{entry.degree}</h3>
